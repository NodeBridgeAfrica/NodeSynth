...
print_node_info() {
  ...
  nodesynth_commit=$(git -C "${BASE_DIR}" rev-parse HEAD)
  nodesynth_version=$(grep EP_VERSION= $BASE_DIR/nodesynth.sh | sed 's/EP_VERSION=//g')
...
NodeSynth Version:  $nodesynth_version
NodeSynth Commit :  $nodesynth_commit
EOF
)
