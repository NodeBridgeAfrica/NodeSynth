...
# Description: NodeSynth is a one-liner setup tool and node management TUI
#
...
# VARIABLES
export BASE_DIR="$HOME/git/nodesynth" && cd $BASE_DIR
...
while true; do
    ...
    CHOICE=$(whiptail --clear --cancel-button "Quit"\
      --backtitle "$BACKTITLE" \
      --title "NodeSynth $EP_VERSION | $NODE_MODE" \
...
while true; do
    ...
    case $SUBCHOICE in
      ...
      10)
        ...
        whiptail --title "Updated NodeSynth" --msgbox "Restart NodeSynth for latest version." 10 78
        ;;
      11)
        MSG_ABOUT="🫰 Created from Public Good of CoinCashew.eth since Pre-Genesis 2020
        \n🫶 Make improvements and suggestions on GitHub: https://github.com/nodebridgeafrica/nodesynth
        ...
        \nOur donation address is 0x or nodebridgeafrica.eth"
        whiptail --title "About NodeSynth" --msgbox "$MSG_ABOUT" 20 78
        ;;
      20)
        configureAutoStart
      ;;
...
while true; do
    ...
    case $SUBCHOICE in
      ...
      9)
        ...
        sudo /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/nodebridgeafrica/client-switcher/master/install.sh)"
        ;;
      10)
...
function getBackTitle(){
    ...
    if [[ $NODE_MODE == "Validator Client Only" ]]; then
        BACKTITLE="${NETWORK_TEXT}${EL_TEXT} | ${CL_TEXT}${VC_TEXT} | Public Goods by nodebridgeafrica.eth"
    else
        BACKTITLE="${NETWORK_TEXT}${EL_TEXT} | $CL_TEXT | $CL-$EL$VC_TEXT | Public Goods by nodebridgeafrica.eth"
    fi
}

function checkV1StakingSetup(){
  if [[ -f /etc/systemd/system/eth1.service ]]; then
    echo "NodeSynth is only compatible with V2 Staking Setups. Using NodeSynth, build a new node in minutes after wiping system or uninstalling V1."
    exit
  fi
}
